...
bool TEST_IsVectorInTree(vector<string> v, BTree *b);
bool isWordInVector (vector<string> v, string read);
bool TEST_CompareReadVectorToTree (vector<string> vReads, vector<string> vWord, BTree *ptr );
...
int main()
{
   ...
   TEST_IsVectorInTree(words , b);
    ...
    //
    TEST_CompareReadVectorToTree (reads, words, b );
...
bool TEST_IsVectorInTree(vector<string> v, BTree *b)
{
    //if everything i put in is still in the tree everything worked, so this checks to see if everything is ok
   for(size_t j=0;j<v.size();j++)
   {
       if(!b->is_word_in_tree(v[j]))
       {
           return false;
           cout<<"something went wrong with setting the tree!"<<endl;
       }
   }
   return true;
}

//look through the vector one element at a time to see if it exists; if it is return true if not return false and cout
bool isWordInVector (vector<string> v, string read)
{
    for (size_t i; i<v.size(); i++) //loop through
    {
            if(v[i]==read)
            {
                return true;
...
bool TEST_CompareReadVectorToTree (vector<string> vReads, vector<string> vWord, BTree *ptr )
{
    bool vectorRet;
    bool treeRet;
    for (size_t i; i<vReads.size(); i++)
    {
        vectorRet = isWordInVector(vWord, vReads[i]); //check in vector
        treeRet=-ptr->is_word_in_tree(vReads[i]); //check in tree

        if(vectorRet!=treeRet)
        {
            cout<< "There's a problem; the tree and read vector do not match. "<<endl;
            return false;
        }
    }
    return true;
